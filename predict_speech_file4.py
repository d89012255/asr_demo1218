...
def post_process_1(input):
    ...
    mix = [   no,dot,positive,yes,zero,one,two,three,four,five,six,eight,nine,seven,negative]
    table= ["否","點","正","是","零","一","二","三","四","五","六","八","九","七","負"]
...
def post_process_3(input):
    ...
    first_team = ["i","yi","u"]
    first_record = ["i","yi","i"]
    mix = [second_record,third_record,forth_record,fifth_record,sixth_record, ninth_record,seventh_record,eighth_record,
    last_dot,next_dot,last_page,last_team,last_record,next_page,next_team,next_record,first_team,first_record]
    table= ["第二筆","第三筆","第四筆","第五筆","第六筆","第九筆","第七筆","第八筆","上一點","下一點","上一頁","上一組",
    "上一筆","下一頁","下一組","下一筆","第一組","第一筆"]
    all = ""
    for i in range(len(input)):
        all+=(input[i][:-1]+" ")
    temp_for_check = all
