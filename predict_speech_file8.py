...
def add_silence_to_wav_file(input_file, output_file, silence_duration_sec):
    ...
    # 將空白音訊資料加到音訊的前後
    new_audio_data = np.concatenate((audio_data, silence_data))
...
def post_process_4(input):
    ...
    mix = [first_team,second_team,third_team,forth_team,fifth_team,sixth_team]
    table= ["第一組","第二組","第三組","第四組","第五組","第六組"]
    all = ""
    for i in range(len(input)):
        all+=(input[i][:-1]+" ")
    temp_for_check = all
